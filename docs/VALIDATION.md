...
- `npm test` (`node --test tests/*.test.mjs`): **111 passed, 0 failed**.
  ...
  - 66 upgrade tests in six new files cover:
    - **impactor classes** (13): six physical classes + historical reference, per-class density/velocity envelopes, `entryCouplingFactor` (cometary 0.88 / rubble 0.93 / metallic 1.03 / others 1.0), `compositionString`, safe-failure validation (`RangeError` wording), azimuth normalization, and export/import round trip through the class fields.
